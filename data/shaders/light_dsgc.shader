/*	light shader (diffuse, specular, gloss, cubemap modulation)
...
PARAM light_transform[4] = { state.matrix.program[1].transpose };
...
TEMP temp;
SUB dir, xyz, light;
DP3 temp.x, transform[0], dir;
DP3 temp.y, transform[1], dir;
DP3 temp.z, transform[2], dir;

DP3 result.texcoord[3].x, light_transform[0], temp;
DP3 result.texcoord[3].y, light_transform[1], temp;
DP3 result.texcoord[3].z, light_transform[2], temp;
...
TEX H0, f[TEX3], TEX3, CUBE;			// modulate texture
MULX H4, H4, H0;						// (diffuse * base + specular) * attenuation * modulate
MOVX H4.w, 1.0;
MULX o[COLH], H4, f[COL0];				// (diffuse * base + specular) * attenuation * modulate * color
...
TEX temp, fragment.texcoord[3], texture[3], CUBE;
MUL result.color, color, temp;
